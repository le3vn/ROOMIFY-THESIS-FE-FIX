...
const HistoryDetailPage: Page = () => {
    ...
    const [bookingDesc] = useState<string | undefined>('');
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [idToModal] = useState<string | undefined>('');
                    ...
                    <div className='flex flex-col h-full justify-between gap-4'>
                        ...
                        <div className='bg-white rounded-lg h-[150px] p-4'>
                            <div className='h-full flex flex-col items-center justify-between'>
                                ...
                                <img src={booking?.bookerMinioUrl} alt="" className='w-8 h-8 rounded-full border-2 border-purple-950 object-cover' />
                                    ...
                                    {booking?.sessionList?.length ? (
                                        <div className='flex flex-col mt-6 h-full justify-end'>
                                            <div className='flex flex-col gap-2 h-4/5'>
                                                {/* Filter approver history to show only the current user's approver details */}
                                                {booking?.approverHistory
                                                    .filter(approver => approver.approverUserId === userId) // Filter by current userId
                                                    .map((approver) => (
                                                        <div key={approver.approverUserId}>
                                                            <div className='flex gap-2 items-center'>
                                                                <img 
                                                                    src={approver.approverMinioUrl} 
                                                                    alt={approver.approverUserName} 
                                                                    className={`w-8 h-8 rounded-full border-2 ${approver.statusId == 1 ? 'border-yellow-600' : approver.statusId == 2 ? 'border-green-600' : 'border-red-600'}`}
                                                                />
...
HistoryDetailPage.layout = WithDefaultLayout;
