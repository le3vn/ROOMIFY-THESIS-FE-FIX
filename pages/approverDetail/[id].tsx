...
import RejectBookingConfirmationModal from '@/components/Modals/ApproversModals/RejectModals/RejectBookingConfirmationModal';
import RejectSuccessModal from '@/components/Modals/ApproversModals/RejectModals/RejectSuccessModal';
...
const RoomDetailPage: Page = () => {
  ...
  const [rejectMessage, setRejectMessage] = useState('');
    ...
    const handleApprove = async () => {
        ...
        const response = await fetchPOST(BackendApiUrl.approveBooking, payload);
        if (response) {
      ...
      const handleRejectBookingConfirmation = () => {
        setIsRejectBookingConfirmationModalOpen(false); // Close the confirmation modal
        setIsRejectModalOpen(true); // Show the reject modal
      };
    
      const handleRejectBookingCancel = () => {
        setIsRejectBookingConfirmationModalOpen(false); // Close the confirmation modal
      };

      const handleRejectMessageChange = (value: string) => {
      ...
      const handleReject = async () => {
        ...
        plainText.innerHTML = rejectMessage;
        const textContent = plainText.textContent || plainText.innerText;
      
        const payload = {
          userId,
          bookingId: booking?.bookingId,
          isApproved: false,
          rejectMessage: textContent.trim(),
        };
                            ...
                            <div className='text-center flex flex-col items-center h-full justify-center'>
                                <div className='h-full'>
                                    ...
                                    <div className='flex flex-col items-center'>
                                        <p className='text-3xl font-semibold text-purple-950'>{booking?.roomName}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className='flex h-2/5 w-full gap-4'>
                        ...
                        <div className='h-full flex flex-col'>
                            ...
                            <div  className='flex justify-center gap-4'>
                                <button className='bg-purple-950 w-[120px] py-2 rounded-full text-white font-semibold' onClick={() => setIsApproveModalOpen(true)}>Approve</button>
                                <button className='bg-red-600 w-[120px] py-2 rounded-full text-white font-semibold' onClick={() => setIsRejectBookingConfirmationModalOpen(true)}>Reject</button>
                            </div>
                            </div>
                        </div>
                        </div>
    ...
    />
      ...
      <RejectBookingConfirmationModal
        isOpen={isRejectBookingConfirmationModalOpen}
        onConfirm={handleRejectBookingConfirmation}
        onCancel={handleRejectBookingCancel}
      />
        ...
        onRejectMessageChange={handleRejectMessageChange}
        />
        <RejectSuccessModal
        isOpen={isRejectSuccessModalOpen}
        onClose={handleRejectSuccessModalClose}
      />
        </div>
    );
};
